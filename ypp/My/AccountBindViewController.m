...
- (void)viewDidLoad {
    ...
    NSString *idnumber = [user objectForKey:@"idnumber"];
    NSString *membershipcardnumber = [user objectForKey:@"membershipcardnumber"];
    NSString *cardname = [user objectForKey:@"cardname"];
    
    _idnumberTextField.text = idnumber;
    _membershipcardnumberTextField.text = membershipcardnumber;
    _cardnameTextField.text = cardname;
...
- (IBAction)submit:(id)sender {
    ...
    if ([_idnumberTextField.text isEqualToString:@""]) {
        [self showHint:@"请输入证件号！"];
        return;
    }
    if (![Util validateIdentityCard:_idnumberTextField.text]) {
        [self showHint:@"请输入正确的证件号！"];
        return;
    }
    if ([_membershipcardnumberTextField.text isEqualToString:@""]) {
        [self showHint:@"请输入会员卡号！"];
        return;
    }
    //验证验证码是否为空
    ...
    [parameters setValue:_cardnameTextField.text forKey:@"cardname"];
    [parameters setValue:_idnumberTextField.text forKey:@"idnumber"];
    [parameters setValue:_membershipcardnumberTextField.text forKey:@"membershipcardnumber"];
    
    NSString *urlString = [NSString stringWithFormat:@"%@%@",HOST,API_SETCARD];
    AFHTTPRequestOperationManager *manager = [AFHTTPRequestOperationManager manager];
    manager.requestSerializer = [AFHTTPRequestSerializer serializer];
    manager.responseSerializer = [AFHTTPResponseSerializer serializer];
    manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json",@"text/json", @"text/plain", @"text/html", nil];
